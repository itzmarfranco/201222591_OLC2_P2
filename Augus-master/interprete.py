...
import gramatica_desc as gDesc
import ts as TS
...
def procesar_variable(tipoVar,ts) :
    ...
    if val.tipo==TS.TIPO_DATO.ARREGLO:
        ...
        return None
    #if val.tipo==TS.TIPO_DATO.REFERENCIA:
    #    simbolo_interno=val.valor
    #    if simbolo_interno.tipo==TS.TIPO_DATO.REFERENCIA:
    #        consola.insert('end','>>Error: No se pueden imprimir arreglos '+val.id+'\n>>')
    #        newErr=ErrorRep('Semantico','No se pueden imprimir arreglos: '+val.id,0)
    #        LisErr.agregar(newErr)
    #        print('Error: No se pueden imprimir arreglos')
    #    return simbolo_interno.valor
    return val.valor
...
def procesar_definicion_arr(asig,ts):
    ...
    if simbolo is None :        
        ...
        #if simbolo.tipo==TS.TIPO_DATO.ARREGLO:
        #    ts.actualizar(tipo_var.id,TS.TIPO_DATO.ARREGLO,{})
        #else :
        #    print('Error: Cambio de tipo de dato -> array')
...
def inicializarEjecucionDescendente(contenido) :
    global LisErr, instrucciones, ts_global

    ts_global = TS.TablaDeSimbolos()
    instrucciones = gDesc.parse(contenido,LisErr)

def inicializarGUI(txteditor,txtconsola):
...
def generarReporteGramaticalDesc():
    gDesc.dot.view()
